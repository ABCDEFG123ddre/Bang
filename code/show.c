...
void PrintInfo(player p[]){
	...
	{
		if(p[i].role==0 || p[i].role==6)
			printf(" Sheriff (%d)    ", p[i].health);
		else if(p[i].alive == 0)
		{
			if(p[i].role==1)
				printf(" Renegade (%d)   ", p[i].health);
			else if(p[i].role==2 || p[i].role==3)
				printf("  Outlaw (%d)    ", p[i].health);
			else if(NumberOfPlayer==5 && p[i].role==4)
				printf("Deputy Sheriff (%d)", p[i].health);
			else if(NumberOfPlayer>=6 && p[i].role==4)
				printf("  Outlaw (%d)    ", p[i].health);
			else if(NumberOfPlayer>=6 && p[i].role==5)
				printf("Deputy Sheriff (%d)", p[i].health);
		}
		else if(p[i].alive == 1)
		{
			printf(" xxxxxxxx (%d)   ", p[i].health);
		}
	}
